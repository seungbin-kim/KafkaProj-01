...
public class ConsumerPartitionAssignSeek {
    ...
    public static void main(String[] args) {
        ...
        props.setProperty(ConsumerConfig.GROUP_ID_CONFIG, "group_pizza_assign_seek_v001"); // 유지보수 작업이라면 바꿔주자
        ...
        kafkaConsumer.seek(topicPartition, 5L); // 만약 읽을 offset이 없으면 처음부터 읽는다.
...
//        pollManualCommitAsync(kafkaConsumer, autoCommit);
    ...
    private static void pollWhileTemplate(KafkaConsumer<String, String> kafkaConsumer,
                                          boolean autoCommit, boolean sync, boolean noCommit) {
        int loopCount = 0;
        ...
        try {
            while (true) {
                ...
                if (autoCommit) {
                    ...
                    try {
                        // 다시 읽은것을 commit 하므로 group_id 다르게 하거나 commit 을 찍지 말자
                        if (noCommit) continue;
                        if (records.count() > 0) {
